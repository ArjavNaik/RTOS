...
int main(void)
{
	...
	sprintf(usr_msg,"B_Sema Example \r\n");
	printmsg(usr_msg);
	...
	work_q = xQueueCreate(1, sizeof(unsigned int));

	vSemaphoreCreateBinary( xSema );


	if( (work_q != NULL) && (xSema != NULL)){
	// 3. Creating tasks
	xTaskCreate(vTask1_handler, "Task-1", 500, NULL, 3, &xTaskHandle1);
	xTaskCreate(vTask2_handler, "TASK-2", 500, NULL, 1, &xTaskHandle2);
...
void vTask1_handler(void *param){
	...
	while(1){
		xWorkTicketId = ( rand() & 0x1FF );
				...
				xStatus = xQueueSend( work_q, &xWorkTicketId , portMAX_DELAY ); //Post an item on back of the queue
...
void vTask2_handler(void *param){
	...
	while(1){
		...
		xStatus =  xQueueReceive(work_q, &xWorkTicketId, 0);
